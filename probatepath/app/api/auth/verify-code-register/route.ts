...
export async function POST(request: Request) {
  ...
  const entry = await prisma.emailLog.findFirst({
    where: { to: email, template: "verification-code" },
  ...
  if (!ok) {
    ...
    try {
      await logSecurityAudit({
        req: request,
        userId: user?.id ?? null,
        action: "auth.verification_code_failed_register",
        meta: { email, attempts: attempts + 1 },
      });
    } catch {}
    return NextResponse.json({ error: "Invalid" }, { status: 400 });
  }
  ...
  try {
    await logSecurityAudit({
      req: request,
      userId: user?.id ?? null,
      action: "auth.verification_code_verified_register",
      meta: { email },
    });
  } catch {}

  // Return success - registration can proceed
