...
const TYPE_COLORS: Record<string, string> = {
  ...
  system: "bg-gray-100 text-[color:var(--text-secondary)]",
};
...
export default function NotificationsPage() {
  ...
  return (
    ...
    >
      {loading ? (
        <div className="text-center py-12 text-sm text-[color:var(--text-secondary)]">Loading notifications...</div>
      ) : notifications.length === 0 ? (
        <div className="rounded-2xl border border-[color:var(--border-muted)] bg-white p-8 text-center">
          <svg className="mx-auto h-10 w-10 text-[color:var(--text-tertiary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
          </svg>
          <p className="mt-3 text-sm text-[color:var(--text-tertiary)]">No notifications yet. We'll notify you when something needs your attention.</p>
        </div>
      ) : (
              ...
              >
                <div className="flex items-start gap-3 px-4 py-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`text-sm ${isUnread ? "font-semibold text-[color:var(--ink)]" : "text-[color:var(--text-secondary)]"}`}>
                      {n.title}
                    </p>
                    <p className="text-xs text-[color:var(--text-tertiary)] mt-0.5">{n.body}</p>
                  </div>
