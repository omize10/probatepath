...
export function GrantUploadForm({ caseId, existingUrl }: GrantUploadFormProps) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-sm text-[color:var(--text-secondary)]">Optional: Upload a copy of your grant document for your records.</p>
      <div>
        <label className="relative cursor-pointer">
          ...
          <span
            className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition ${
              uploading
                ? "bg-gray-200 text-[color:var(--text-secondary)] cursor-not-allowed"
                : "border border-gray-300 bg-white text-[color:var(--text-secondary)] hover:bg-gray-50"
            }`}
          >
            {uploading ? (
              <>
        ...
        <p className="mt-1 text-xs text-[color:var(--text-tertiary)]">PDF, JPG, or PNG up to 10MB</p>
      </div>
