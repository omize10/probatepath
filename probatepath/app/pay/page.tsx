...
import { CreditCard, Rocket, Loader2, CheckCircle2, Phone } from "lucide-react";
...
export default function PayPage() {
  ...
  // Validate token and get prefill data on mount
  useEffect(() => {
  ...
  const handleSubmit = async (skipped: boolean = false) => {
    ...
    try {
      // First, create or get tier selection
      ...
      const { nextRoute } = await paymentResponse.json();

      // Clear any session storage from other flows
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[color:var(--bg-canvas)]">
        <div className="text-center space-y-4">
          ...
          <p className="text-[color:var(--muted-ink)]">Validating your link...</p>
        </div>
      </div>
    );
  }
        ...
        <Card className="max-w-md w-full border-[color:var(--border-muted)]">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-[color:var(--muted-ink)] text-center">
              This payment link has expired or already been used. Please start over or contact us for help.
            </p>
      ...
      <div className="mx-auto max-w-4xl space-y-8">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-green-800">
                  We've pre-filled your information from our conversation. Review and complete payment to continue.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4 text-center">
          ...
          <p className="mx-auto max-w-2xl text-base text-[color:var(--muted-ink)]">
            {prefillData?.grantType === "administration"
                    ...
                    {(["basic", "standard", "premium"] as Tier[]).map((tier) => (
                      <button
                        ...
                        className={`rounded-xl border-2 p-4 text-left transition ${
                          selectedTier === tier
                            ? "border-[color:var(--brand)] bg-[color:var(--bg-surface)]"
                            : "border-[color:var(--border-muted)] hover:border-[color:var(--brand)]/50"
                        }`}
                      >
                        <p className="font-semibold capitalize text-[color:var(--brand)]">{tier}</p>
            ...
            <Card className="border-[color:var(--border-muted)] shadow-[0_25px_60px_-50px_rgba(15,23,42,0.18)]">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  ...
                  {prefillData?.grantType && (
                    <div className="flex justify-between text-sm">
                      <span className="text-[color:var(--muted-ink)]">Type</span>
                      <span className="capitalize text-[color:var(--brand)]">
                        {prefillData.grantType}
                      </span>
                    </div>
                  )}
                ...
                <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-xs text-blue-800">
                  ...
                  <p className="mt-1">
                    Your card will not be charged during the beta period.
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-[color:var(--border-muted)] bg-[color:var(--bg-muted)] p-4 text-xs text-[color:var(--muted-ink)]">
                  ...
                  <p className="mt-1">
                    {selectedTier === "basic"
                      ? "After payment, continue to the intake questionnaire to enter your estate details."
                      : "After payment, schedule a call with our team. We'll walk you through everything."}
                  </p>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <a href="tel:+16046703534" className="text-sm text-[color:var(--muted-ink)] hover:underline">
                      Call (604) 670-3534
                    </a>
                  </div>
                </div>
