...
export function generateP10HTML(data: P10Data): string {
  ...
  const realPropertyOutsideTotal = data.assets?.realPropertyOutsideBC?.reduce((sum, item) => sum + (item.value || 0), 0) || 0;
  const tangibleOutsideTotal = data.assets?.tangiblePersonalPropertyOutsideBC?.reduce((sum, item) => sum + (item.value || 0), 0) || 0;
  const grossValueOutside = realPropertyOutsideTotal + tangibleOutsideTotal;
...
<head>
  ...
  <title>Form P10 - Affidavit of Assets and Liabilities for Domiciled Estate Grant</title>
  <style>
    @page {
      size: 8.5in 11in;
      margin: 0.5in 0.75in;
...
<div class="form-header">
  <div class="form-number">Form P10 (Rule 25-3 (2))</div>
</div>
...
<div class="form-title">Affidavit of Assets and Liabilities for Domiciled Estate Grant</div>
...
<!-- Paragraph 4 -->
<div class="paragraph">
  <span class="instruction">[Check the box for whichever one of the immediately following section 4's is correct and provide any required information.]</span>
</div>

<div class="checkbox-item">
  <span class="checkbox">${checkbox(data.hasPropertyOutsideBC)}</span>
  <span>Attached to this affidavit as Exhibit B is a Statement of Real and Tangible Property Outside of British Columbia that discloses</span>
</div>

<div class="sub-paragraph">(a) the real property and tangible personal property outside of British Columbia that passes to the applicant in the applicant's capacity as the deceased's personal representative,</div>
<div class="sub-paragraph">(b) the value of that property, and</div>
<div class="sub-paragraph">(c) the liabilities that charge or encumber that property.</div>

<div class="checkbox-item">
  <span class="checkbox">${checkbox(!data.hasPropertyOutsideBC)}</span>
  <span>There is no real property or tangible personal property outside of British Columbia that passes to the applicant in the applicant's capacity as the deceased's personal representative.</span>
</div>
    ...
    <tbody>
      <tr>
        <td colspan="2" class="example-text">
          List item details and then list secured debt details below those items. If the deceased only has a partial interest in the property, list the names of all registered owners, including the deceased, and specify the interest that each owner has in the property as a fraction expressed in numerals.<br><br>
          For example: 123 Main Street, Victoria, BC<br>
...
<!-- EXHIBIT B (if property outside BC) -->
${data.hasPropertyOutsideBC ? `
<div class="exhibit-section">
  <div class="exhibit-mark">Exhibit B</div>
  
  <div class="exhibit-header">
    <table>
      <tr>
        <td style="border-left: 1px solid #000; padding-left: 12pt;">
          <div>This is Exhibit B referred to in the affidavit of</div>
          <div style="border-bottom: 1px solid #000; min-width: 200pt; margin: 6pt 0;">&nbsp;</div>
          <div>before me on <span style="border-bottom: 1px solid #000; min-width: 120pt; display: inline-block;">&nbsp;</span></div>
          <div style="margin-top: 24pt; border-bottom: 1px solid #000; min-width: 200pt;">&nbsp;</div>
          <div>A commissioner for taking affidavits for</div>
  ...
  <div class="statement-title">Statement of Real and Tangible Property Outside of British Columbia</div>
  ...
  <table class="asset-table">
    ...
    <tbody>
      ...
      ${data.assets?.realPropertyOutsideBC?.map(property => `
      <tr>
        <td>
          ${property.address || property.description || ''}<br>
          ${property.owners ? `Registered owners: ${property.owners}` : ''}
        </td>
        <td class="value-col">${formatCurrency(property.value || 0)}</td>
      </tr>
      `).join('') || ''}
      <tr class="total-row">
        <td>TOTAL REAL PROPERTY OUTSIDE BRITISH COLUMBIA</td>
        <td class="value-col">${formatCurrency(realPropertyOutsideTotal)}</td>
      </tr>
    </tbody>
  </table>
  
  <!-- Part II - Tangible Personal Property Outside BC -->
  <div style="margin-top: 18pt; font-weight: bold;">Part II Tangible Personal Property outside British Columbia (including vehicles, furniture and other physical items)</div>
  ...
  <table class="asset-table">
    ...
    <tbody>
      <tr>
        <td colspan="2" class="example-text">List item details; it is unnecessary to list secured debts</td>
      </tr>
      ${data.assets?.tangiblePersonalPropertyOutsideBC?.map(item => `
      <tr>
        <td>${item.description}</td>
        <td class="value-col">${formatCurrency(item.value || 0)}</td>
      </tr>
      `).join('') || ''}
      <tr class="total-row">
        <td>TOTAL TANGIBLE PERSONAL PROPERTY OUTSIDE BRITISH COLUMBIA</td>
        <td class="value-col">${formatCurrency(tangibleOutsideTotal)}</td>
      </tr>
    </tbody>
  </table>
  
  <!-- Gross Value Outside BC -->
  <table class="asset-table" style="margin-top: 18pt;">
    <tbody>
      <tr class="total-row">
        <td>GROSS VALUE OF ASSETS OUTSIDE BRITISH COLUMBIA</td>
        <td class="value-col">${formatCurrency(grossValueOutside)}</td>
      </tr>
    </tbody>
  </table>
</div>
